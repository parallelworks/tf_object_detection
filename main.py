...
if __name__ == '__main__':
    ...
    print(datetime.now(), 'Mount command:\n' + mount_cmd, flush = True)

    futs = []
    imgdirs = pwargs.imgdirs.split('---')

    for imgdir in imgdirs:
        print(datetime.now(), 'Processing images in directory: ' + imgdir, flush = True)
    ...
    for fut,imgdir in zip(futs, imgdirs):
        print(datetime.now(), 'Waiting for images in: ' + imgdir, flush = True)
        fut.result()

    # Merge results for design explorer:
    print(datetime.now(), 'Creating Design Explorer files', flush = True)
    dex_fut = merge_dex_results(
        pwargs.worker_mount_dir,
        pwargs.pw_mount_dir,
        imgdirs,
        outputs = [ Path('./dex.csv'), Path('./dex.html')]
    )
    ...
    if len(imgdirs) == 1:
        dex_fut.result()
        print(datetime.now(), 'Design Explorer files are ready', flush = True)

    # Merge measurements:
    print(datetime.now(), 'Merging measurements', flush = True)
    measurements_fut = merge_measurements(
        pwargs.worker_mount_dir,
        imgdirs,
        outputs = [Path('./measurements.csv')]
    )
    ...
    print(datetime.now(), 'Measurements are ready', flush = True)
